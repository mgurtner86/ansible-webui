...
-- Description: Add support for customizable email templates and user notification preferences
...
CREATE POLICY "Authenticated users can update own email templates"
  ON email_templates FOR UPDATE
  TO authenticated
  USING (auth.uid() = created_by OR auth.uid() = updated_by)
  WITH CHECK (auth.uid() = updated_by);

CREATE POLICY "Authenticated users can delete own email templates"
