    ...
    <Layout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-lg">Overview of your Ansible Tower instance</p>
        </div>
          ...
          {statCards.map((card) => {
            ...
            return (
              <Link
                key={card.name}
                to={card.link}
                className="group relative overflow-hidden bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 p-8 border border-slate-200/60"
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-5xl font-light text-slate-900 mt-3">{card.value}</p>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-md p-8 border border-slate-200/60">
            <h2 className="text-2xl font-light text-slate-800 mb-6 tracking-tight">Job Status</h2>
            <p className="text-sm text-slate-500 mb-6">Last 24 hours</p>
            <div className="grid grid-cols-2 gap-4">
              {jobStatusCards.map((card) => {
                const Icon = card.icon;
                return (
                  <div key={card.name} className="group relative overflow-hidden bg-gradient-to-br from-slate-50 to-white rounded-xl p-6 border border-slate-200/60 hover:shadow-md transition-all duration-300">
                    <div className={`absolute inset-0 bg-gradient-to-br ${card.color} opacity-0 group-hover:opacity-5 transition-opacity duration-300`}></div>
                    <div className="relative">
                      ...
                      <p className="text-3xl font-light text-slate-900 mt-2">{card.value}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-white rounded-xl shadow-md p-8 border border-slate-200/60">
            <h2 className="text-2xl font-light text-slate-800 mb-6 tracking-tight">Recent Jobs</h2>
              ...
              {stats?.recentJobs.length === 0 ? (
                ...
                stats?.recentJobs.map((job: any) => (
                  <Link
                    key={job.id}
                    to={`/jobs/${job.id}`}
                    className="group block p-4 border border-slate-200/60 rounded-xl hover:shadow-md hover:border-slate-300 transition-all duration-200 bg-gradient-to-r from-white to-slate-50"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-900 truncate group-hover:text-blue-600 transition-colors">{job.template_name}</p>
