        ...
        {showForm && (
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8 border border-slate-200/60 dark:border-slate-700/60">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Git URL</label>
                <input
                  type="url"
                  required
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div key={project.id} className="group bg-white dark:bg-slate-800 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 p-6 border border-slate-200/60 dark:border-slate-700/60">
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  ...
                  className="p-2 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all"
                  title="Sync project"
                >
                  <RefreshCw className="w-5 h-5" />
                </button>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 line-clamp-2 leading-relaxed">{project.description}</p>
              <div className="space-y-3">
                <div className="flex items-center text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-700 px-3 py-2 rounded-lg">
                  <GitBranch className="w-4 h-4 mr-2 text-slate-400 dark:text-slate-500" />
                  <span className="font-mono text-xs">{project.git_branch}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-600 dark:text-slate-400 font-medium">
                    <span className="text-2xl font-light text-slate-900 dark:text-slate-100">{project.playbook_count}</span> playbooks
                  </span>
          ...
          {projects.length === 0 && !showForm && (
            <div className="col-span-full text-center py-16">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-lg">No projects yet. Create one to get started.</p>
            </div>
          )}
        </div>
      </div>
